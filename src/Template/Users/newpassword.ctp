...
<div class="users form  content">
    ...
    <fieldset>
        ...
        <legend>  <?= $this->html->image("key.ico",["width"=>"10%"]) ?>  <?= __('Nouveau mot de passe : ') ?></legend>
        <?php

           
            echo $this->Form->control('newpassword',["type"=>"password","label" => "NOUVEAU MOT DE PASSE : ","required"=>"true","onchange"=>"verify_password()"]);
            echo $this->Form->control('newpasswordcheck',["type"=>"password","label" => "CONFIRMER NOUVEAU MOT DE PASSE : ","required"=>"true","onchange"=>"verify_password()"]);
        ?>
